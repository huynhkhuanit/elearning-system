...
function validateVideoUrl(url: string | null): {
  ...
  if (!url) {
    return { isValid: false, url: null, type: 'none', reason: 'No URL provided' };
  }
  ...
  if (url.includes('youtube.com') || url.includes('youtu.be')) {
    ...
    if (!videoId) {
      return { isValid: false, url, type: 'youtube', reason: 'Invalid YouTube URL format' };
    }
  ...
  if (url.includes('vimeo.com')) {
    ...
    if (!videoId || isNaN(Number(videoId))) {
      return { isValid: false, url, type: 'vimeo', reason: 'Invalid Vimeo URL format' };
    }
  ...
  // Unknown format
  return { isValid: false, url, type: 'unknown', reason: 'Unrecognized URL format' };
}
