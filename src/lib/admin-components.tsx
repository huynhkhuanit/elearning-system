  ...
  return (
    <div className="bg-gradient-to-br from-slate-800 to-slate-900 border border-slate-700 rounded-lg p-6 hover:border-slate-600 transition-all duration-300 group">
      <div className="flex items-center justify-between mb-4">
        <div className={`p-3 ${colors.bg} rounded-lg group-hover:${colors.bg} transition`}>
      ...
      <div className="space-y-2">
        ...
        {trend && (
          <p className={`text-sm font-medium ${trend.isPositive ? 'text-green-400' : 'text-red-400'}`}>
            {trend.isPositive ? '+' : ''}{trend.value}% {trend.isPositive ? 'naik' : 'hạ'}
          </p>
        )}
      </div>
    </div>
...
export function Alert({ type, title, message, icon: Icon }: AlertProps) {
  ...
  return (
    <div className={`p-4 ${config.bg} border ${config.border} rounded-lg flex items-start gap-3`}>
      {Icon && <Icon className={`w-5 h-5 ${config.icon} flex-shrink-0 mt-0.5`} />}
...
export function Button({
  ...
  const variantClass = {
    ...
    secondary: 'bg-slate-700 hover:bg-slate-600 text-slate-100 disabled:bg-slate-700/50',
...
const statusConfig = {
  ...
  published: { bg: 'bg-emerald-500/20', text: 'text-emerald-300', border: 'border-emerald-500/30' },
